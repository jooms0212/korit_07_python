...
def process_coins():
    ...
    sum = 0
    # 이부분에 로직이 들어가야 할겁니다
    # quarter = float(input('쿼터 동전을 몇개나 넣으시겟습니까? >>>'))*0.25
    # dime = float(input('다임 동전을 몇개나 넣으시겟습니까? >>>'))*0.1
    # nickel = float(input('니켈 동전을 몇개나 넣으시겟습니까? >>>'))*0.05
    # penny = float(input('페니 동전을 몇개나 넣으시겟습니까? >>>'))*0.01
...
def make_coffee(drink_name, order_ingredient):
    ...
    '''
    # resources가 아니라 order_ingredient를 반복문 돌리는 이유는 에스프레소일 때 오류가 발생하기 때문입니다. 에스프레소의 재료에는 우유가 없는데
    # resources에는 있는 상태가 되죠. 그래서 order_ingredient['우유'] 부분에서 오류가 발생하게 됩니다.
    for item in order_ingredient:
        resources[item] -= order_ingredient[item]
    print(f'{drink_name}이(가) 나왔습니다. 맛있게 드세요 !!')
#todo - 2 : 커피 머신이 반복적으로 돌아가야하는데, off를 입력 받으면 종료가 이루어져야 합니다.
...
#todo - 5 : choice가 이상의 조건을 충족하지 않을 때 '잘못 입력했습니다. 다시 입력하세요'를 출력하는 부분
    else:
