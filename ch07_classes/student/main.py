...
1. name이라는 속성이 있는데 객체명.name을 통해서 '김일' / '김영'이라는 속성값이 출력된다는 점.
...
그래서 이거 원래는 자동 생성되기 때문에 일일이 애너테이션 달고 .속성명인지 그냥 속성명인지 따질 필요가 없는데
지금은 좀 쓸모가 없는 상황입니다.

저희는 python으로 백엔드를 짜지 않을거기 때무네 일단 이런 방식으로 setter / getter를 생성한다는 점만 유의해두시고
springboot에 대비하여 Java 방식의 setter / getter를 작성하겠습니다.
'''
