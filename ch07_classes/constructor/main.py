...
그럼 Java에서 수업한 것처럼 속성값이 대입되지 않은 객체를 생성한 다음에 속성값을
집어넣어주는 과정을 거쳐야 합니다.
...
'''
이게 굳이 속성값에 대한 제한이 있지 않다면 빈 객체 만들어놓고 거기에 값 대입하는게 비효율적으로 느껴집니다.
그래서 저희는 생성자를 도입할겁니다.

Java / JS 등에서는 생성자 명은 클래스명과 동일하거나 constructor인데, 또 python만 지혼자서 이상한걸로 생성자를 만듭니다.
'''
class Candy2:
    # 생성자 정의
...
소멸자를 통해서 강제로 객체를 삭제해주면 메모리 관리가 편하겠네요.
...
james is dead
emily is dead
라고 결과값이 나옵니다. 이 이유는 모든 코드블럭이 다 읽어지면 메모리에 할당된 객체는 자동 소멸하기 때문입니다.
